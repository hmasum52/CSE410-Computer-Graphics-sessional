...
Point3D center;    // camera looks at
Point3D r8;         // right vector for camera
...
void rotateObject(bool isAntiClockwise=false){
    ...
    double lx = center.x - eye.x; // look vector x
    double lz = center.z - eye.z; // look vector z

    int dir = isAntiClockwise ? 1 : -1;

    eye.x += v * (dir*up.y*lz);
    eye.z += v * (-dir*lx*up.y);
    double s = sqrt(eye.x*eye.x + eye.z*eye.z) /(sqrt(2));
    eye.x /= s;
    eye.z /= s;
}
...
void specialKeyListener(int key, int x,int y) {
    double v = 0.01;           
    double lx = center.x - eye.x; // look vector x
    double ly = center.y - eye.y; // look vector y
    double lz = center.z - eye.z; // look vector z
    ...
    case GLUT_KEY_DOWN: // move backward
        ...
        eye.z -= v*lz;
        break;
    case GLUT_KEY_LEFT: // move camera right
        eye.x -= v*r8.x;
        eye.y -= v*r8.y;
        eye.z -= v*r8.z;

        break;
    case GLUT_KEY_RIGHT: // move camera left
        eye.x += v*r8.x;
        eye.y += v*r8.y;
        eye.z += v*r8.z;
        break;
    case GLUT_KEY_PAGE_UP: // tilt up(move up)
        eye.x += v*up.x;
        eye.y += v*up.y;
        eye.z += v*up.z;
        break;
    case GLUT_KEY_PAGE_DOWN: // tilt down(move down)
        eye.x -= v*up.x;
        eye.y -= v*up.y;
        eye.z -= v*up.z;
        break;
